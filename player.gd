...
## break (LMB), place (RMB). Hearts (health) with fall damage + void respawn.
...
var _fall_speed := 0.0
var _landed_once := false   # the FIRST landing after (re)spawn never hurts
...
func _physics_process(delta: float) -> void:
	...
	# First landing after (re)spawn is free — terrain chunks may still be
	# streaming in, so the spawn drop never costs hearts. Later falls do.
